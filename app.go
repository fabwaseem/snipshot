...
import (
	...
	"snipshot/internal/config"
	"snipshot/internal/hotkeys"
	"snipshot/internal/screenshot"
	"snipshot/internal/tray"
	winEnum "snipshot/internal/windows"
)
...
func (a *App) startup(ctx context.Context) {
	...
	a.trayIcon = tray.NewTrayIcon("Snipshot - Screenshot Tool")
...
// PrepareRegionCapture prepares for region selection by capturing the specified display and setting up overlay
// If displayIndex is -1, captures the display where the cursor is currently located
func (a *App) PrepareRegionCapture(displayIndex int) (*RegionCaptureData, error) {
...
// SetWidgetMode resizes window to compact widget size
func (a *App) SetWidgetMode() {
	if a.ctx == nil {
		return
	}
	// Set small widget size - wider to fit all buttons
	runtime.WindowSetMinSize(a.ctx, 0, 0)
	runtime.WindowSetSize(a.ctx, 700, 100)
	runtime.WindowCenter(a.ctx)
	runtime.WindowShow(a.ctx)
}

// SetScreenSelectionMode resizes window to fit screen selection modal (only in widget mode)
func (a *App) SetScreenSelectionMode() {
	if a.ctx == nil {
		return
	}
	// Resize to accommodate screen selection modal
	// Width: ~900px for modal, Height: ~600px to show multiple screens
	runtime.WindowSetMinSize(a.ctx, 0, 0)
	runtime.WindowSetSize(a.ctx, 900, 650)
	runtime.WindowCenter(a.ctx)
	runtime.WindowShow(a.ctx)
}

// SetWindowPickerMode resizes window to fit window picker modal (only in widget mode)
func (a *App) SetWindowPickerMode() {
	if a.ctx == nil {
		return
	}
	// Resize to accommodate window picker modal
	// Width: ~800px for modal, Height: ~700px to show multiple windows
	runtime.WindowSetMinSize(a.ctx, 0, 0)
	runtime.WindowSetSize(a.ctx, 800, 700)
	runtime.WindowCenter(a.ctx)
	runtime.WindowShow(a.ctx)
}

// SetEditorMode resizes window to full editor size
func (a *App) SetEditorMode() {
	if a.ctx == nil {
		return
	}
	// Restore min size constraint
...
// CaptureDisplayThumbnail captures a thumbnail preview of a display for UI preview
func (a *App) CaptureDisplayThumbnail(displayIndex int, maxWidth, maxHeight int) (*screenshot.CaptureResult, error) {
	return screenshot.CaptureDisplayThumbnail(displayIndex, maxWidth, maxHeight)
}
...
func (a *App) CaptureDisplay(displayIndex int) (*screenshot.CaptureResult, error) {
	...
	// Restore window state after capture
	// Note: The window will be shown and resized by FinishScreenSelection and SetEditorMode
	// which are called from the frontend after capture
...
func (a *App) QuickSave(imageData string, format string) SaveImageResult {
	...
	// Create Snipshot folder in Pictures
	saveDir := filepath.Join(homeDir, "Pictures", "Snipshot")
	...
	filename := "snipshot_" + timestamp + ext
