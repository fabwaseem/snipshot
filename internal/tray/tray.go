...
func (t *TrayIcon) loadIcon() uintptr {
	// Get module handle
	hInstance, _, _ := procGetModuleHandleW.Call(0)

	// Try loading icon from executable resource (Wails embeds the icon from build/windows/icon.ico)
	// Resource ID 1 is typically the main application icon
	...
	// Fallback to default application icon if resource loading failed
	if hIcon == 0 {
...
func (t *TrayIcon) run() {
	...
	className := syscall.StringToUTF16Ptr("SnipshotTrayClass")
	...
	t.hwnd, _, _ = procCreateWindowExW.Call(
		...
		uintptr(unsafe.Pointer(syscall.StringToUTF16Ptr("Snipshot"))),
		0,
		0, 0, 0, 0,
		0, 0,
		hInstance,
...
func (t *TrayIcon) showMenu() {
	...
	// Add menu items
	appendMenu(hMenu, MF_STRING, MenuShow, "Show Snipshot")
